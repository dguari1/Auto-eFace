...
class Emotrics(QtWidgets.QDialog):
    ...
    def create_new_window(self):
        ...
        if self.displayImage._shape is not None:
            ...
            MeasurementsLeft, MeasurementsRight, MeasurementsDeviation, MeasurementsPercentual,_ = get_measurements_from_data(self.displayImage._shape, self.displayImage._lefteye, self.displayImage._righteye, self._CalibrationType, self._CalibrationValue)
    ...
    def validate_exit(self):
        ...
        if self.displayImage._shape is not None: # only save if there is a shape
            ...
            if not (os.path.isfile(file_txt)):
                ...
                if (shape==self.displayImage._shape).all() and (lefteye==self.displayImage._lefteye) and (righteye==self.displayImage._righteye):
                    #no change move forward
                    pass
                else:
                    buttonReply = QMessageBox.question(self, 'Save changes', "Landamarks positions were modified.<br>Do you want to save the changes?", QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel, QMessageBox.Cancel)
